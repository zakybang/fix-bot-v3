...
handler.all = async function (m) {
	...
	try {
		...
		let wahid = await conn.profilePictureUrl(who, 'image').catch(_ => hwaifu.getRandom())
		let imagea = await new knights.Jo().setImage(wahid).toBuild();
		let dataa = imagea.toBuffer();
		let imageb = await new knights.Patrick().setAvatar(wahid).toAttachment();
		let datab = imageb.toBuffer();
		let imagec = await new knights.Bonk().setAvatar1(wahid).setAvatar2(wahid).toBuild();
		let datac = imagec.toBuffer();
		let imaged = await new knights.Burn().setAvatar(wahid).toAttachment();
       ...
       key: {
        ...
        let pft = [global.ftroli, global.fkontak, global.fvn, global.ftextt, global.fliveLoc, global.fliveLoc2, global.ftoko, global.fdocs, global.fgclink, global.fgif, global.fvid, global.fpoll]
